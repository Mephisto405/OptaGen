...
from vis_feat import ToneMap, LinearToSrgb, normalize_world_pos
...
def gradient_importance_map(img):
    if len(img.shape) == 3 and img.shape[2] == 3:
        ...
        a = img_x_0 * img_x_0 + img_y_0 * img_y_0 + img_x_1 * img_x_1 + img_y_1 * img_y_1 + img_x_2 * img_x_2 + img_y_2 * img_y_2
        check = np.argwhere(a < 0)
        # print(check.shape)
        a = a.clip(0)
        ...
        b = img_x_0 * img_x_0 + img_y_0 * img_y_0
        # check = np.argwhere(b  0)
        # print(check)
        # print(b[check])
        # print(b[b>0].min())
        b = b.clip(0)
...
class DenoiseDataset(Dataset):
    ...
    def __init__(self, gt_base_dir, spp, base_model='sbmc', mode='train', batch_size=8, sampling='random', use_g_buf=True, use_sbmc_buf=True, use_llpm_buf=False, pnet_out_size=3):
        ...
        self.idx_llpm = {
            ...
            'path_weight': [24+(self.MAX_DEPTH+1)*7,
                            25+(self.MAX_DEPTH+1)*7],
            'radiance_wo_weight': [25+(self.MAX_DEPTH+1)*7,
                                   28+(self.MAX_DEPTH+1)*7],
            'light_intensity': [28+(self.MAX_DEPTH+1)*7,
                                31+(self.MAX_DEPTH+1)*7],
            'throughputs': [31+(self.MAX_DEPTH+1)*7,
                            31+(self.MAX_DEPTH+1)*10],
            'roughnesses': [31+(self.MAX_DEPTH+1)*10,
                            31+(self.MAX_DEPTH+1)*11],
            'normals': [31+(self.MAX_DEPTH+1)*11,
                        31+(self.MAX_DEPTH+1)*14],
            'world_positions': [31+(self.MAX_DEPTH+1)*14,
                                31+(self.MAX_DEPTH+1)*17],
        }
...
# Preprocessing
    ...
    def _preprocess_llpm(self, sample):
        ...
        idx_s, idx_e = self.idx_llpm['normals']
        normals = sample[..., idx_s:idx_e]
        # roughnesses = np.sqrt(roughnesses)

        idx_s, idx_e = self.idx_llpm['world_positions']
        world_positions = sample[..., idx_s:idx_e]
        # roughnesses = np.sqrt(roughnesses)
    ...
    def _offline_preprocess(self, llpm=True, sbmc=True, kpcn=True, overwrite=False, remove_original=False):
        ...
        def _offline_preprocess_thread(gt_files, pbar, llpm=True, sbmc=True, kpcn=True, overwrite=False, remove_original=False):
            for gt_fn in tqdm(gt_files, leave=False, ncols=100):
                print(gt_fn)
                ...
                if (llpm):
                    if (not os.path.isfile(llpm_fn) or overwrite):
                        ...
                        print('llpm size', llpm_buffer.shape)
                ...
                if (kpcn):
                    if self.mode == 'test':
                        ...
                        for _s in [2, 4, 8]: # support upto 64 spp due to the lack of gpu memory
                            ...
                            if (not os.path.isfile(kpcn_fn) or overwrite):
                                ...
                                while s < _s:
                                    i += 1
                                    _in_fn = in_fn[:in_fn.rfind('.')] +  '_' + str(i) + in_fn[in_fn.rfind('.'):]
                                    print('kpcn parsing input', _in_fn)
                ...
                if self.mode != 'test':
                    ...
                    if (not os.path.isfile(prob_fn) or overwrite):
                        ...
                        diffuse = sbmc_p_buffer[...,66-27].mean(2) # 75-27 72-27
                        glossy = sbmc_p_buffer[...,71-27].mean(2) # 81-27 78-27
                        specular = sbmc_p_buffer[...,76-27].mean(2) # 87-27 84-27
        ...
        total_len = len(self.gt_files)
        n = 4
        chunks = [self.gt_files[i:i+n] for i in range(0, len(self.gt_files), n)]
        with tqdm(total=total_len) as pbar:
            with ThreadPoolExecutor(max_workers=len(chunks)) as ex:
                for chunk in chunks:
                    ex.submit(_offline_preprocess_thread, chunk, pbar)
...
class FullImageDataset(Dataset):
    ...
    def __init__(self, in_fn, spp, base_model='sbmc', use_g_buf=True, 
        ...
        if base_model == self.KPCN:
            ...
            self.full_ipt = mapping(np.mean(sample['radiance'], 2))
            self.full_tgt = mapping(sample['target_image'])
            self.normal = np.mean(0.5 * sample['features'][...,17:20] + 0.5, 2)
            self.albedo = np.mean(sample['features'][...,11:14], 2)
            self.depth = np.mean(sample['features'][...,21], 2)
            self.visibility = np.mean(sample['features'][...,22], 2)
        ...
        if visualize and use_llpm_buf:
            ...
            self.throughput = mapping(np.mean(sample['paths'], 2)[...,6:21])
            self.bounce_type = np.mean(sample['paths'], 2)[...,21:26] # 24
            self.roughness = np.mean(sample['paths'], 2)[...,26:31] # 30
            self.normals = 0.5 * np.mean(sample['paths'], 2)[...,31:46] + 0.5
            self.positions = np.mean(normalize_world_pos(sample['paths'][...,46:61]), 2)
...
if __name__ == '__main__':
    ...
    """
    def test1():
        dataset = DenoiseDataset('D:\\newdata', 8, mode='test', base_model='sbmc', use_g_buf=True, use_sbmc_buf=True, use_llpm_buf=True, sampling='grid')
        dataloader = DataLoader(dataset, batch_size=8, num_workers=1, pin_memory=False)
        for batch in dataloader:
            for k in batch:
                print(k, batch[k].shape)
                if k == 'paths':
                    res = 25+(dataset.MAX_DEPTH+1)*7
                    paths = batch[k][0]
                    radiance_wo_weight
                    idx_s, idx_e = dataset.idx_llpm['radiance_wo_weight']
                    radiance_wo_weight = paths[:,idx_s-res:idx_e-res,:,:].numpy()
                    radiance_wo_weight = np.exp(radiance_wo_weight* 30.0)
                    print(radiance_wo_weight.shape)
                    img = np.transpose(np.mean(radiance_wo_weight, axis=0), (1, 2, 0))
                    print(img.shape)
                    plt.imshow(img)
                    plt.show()
                    light_intensity
                    idx_s, idx_e = dataset.idx_llpm['light_intensity']
                    light_intensity = paths[:,idx_s-res:idx_e-res,:,:].numpy()
                    # print(light_intensity.shape)
                    light_intensity = np.exp(light_intensity * 10.0)
                    light_intensity = np.transpose(np.mean(light_intensity, axis=0), (1, 2, 0))
                    plt.imshow(LinearToSrgb(ToneMap(light_intensity, 1.5)))
                    plt.show()
                    idx_s, idx_e = dataset.idx_llpm['normals']
                    normals = paths[:,idx_s-res:idx_e-res,:,:].numpy()
                    normals = np.transpose(np.mean(normals, 0) * 0.5 + 0.5)

                    plt.imshow(normals[:,:,:3])
                    plt.show()
                    idx_s, idx_e = dataset.idx_llpm['normals']
                    normals = paths[:,idx_s-res:idx_e-res,:,:].numpy()
                    normals = np.transpose(np.mean(normals, 0) * 0.5 + 0.5)
                    ...
                    idx_s, idx_e = dataset.idx_llpm['world_positions']
                    world_positions = paths[:,idx_s-res:idx_e-res,:,:].numpy()
                    world_positions = world_positions.transpose((2, 3, 0, 1))
                    # print(world_positions)
                    world_positions = normalize_world_pos(world_positions)
                    # normals = np.transpose(np.mean(normals, 0) * 0.5 + 0.5)
                    for i in range(dataset.MAX_DEPTH+1):
                        plt.subplot(2,3,i+1)
                        plt.title(str(i+1))
                        wp = np.mean(world_positions[:,:,:,3*i:3*i+3], 2)
                        # wp = world_positions[:,:,0,3*i:3*i+3]
                        # print(wp)
                        plt.imshow(wp, cmap='magma')
                    plt.show()
            break
    ...
    def test2():
        dataset = DenoiseDataset('D:\\SBMC', 8, mode='test', base_model='sbmc', use_g_buf=True, use_sbmc_buf=True, use_llpm_buf=True, sampling='grid')
        start = time.time()
    ...
    def test3():
        dataset = FullImageDataset('D:\\SBMC\\test\\input\\bathroom.npy', 8, 'kpcn', True, True, True, 3, visualize=True)
        plt.title('has hit')
        plt.imshow(dataset.radiance_wo_weight)
        plt.show()

    def save_all_buffers():
        test_list = ['bathroom', 'car', 'car2', 'chair', 'gharbi', 'tableware', 'teapot']
        train_list = ['bathroom-2_2']
        for s in train_list:
            dataset = FullImageDataset('D:\\SBMC\\train\\input\\%s.npy'%(s), 8, 'sbmc', True, True, True, 3, visualize=True)
            # plt.imshow(np.mean(dataset.depth, 2), cmap='binary')
            # plt.show()
            plt.imsave('D:\\buffers\\%s-noisy.png'%(s), dataset.full_ipt)
            plt.imsave('D:\\buffers\\%s-clean.png'%(s), dataset.full_tgt)
            print(dataset.normal.shape)
            plt.imsave('D:\\buffers\\%s-normal.png'%(s), dataset.normal)
            plt.imsave('D:\\buffers\\%s-depth.png'%(s), dataset.depth)
            plt.imsave('D:\\buffers\\%s-albedo.png'%(s), dataset.albedo)

            plt.imsave('D:\\buffers\\%s-radiance_wo_weight.png'%(s), dataset.radiance_wo_weight, vmin=0.0, vmax=1.0)
            plt.imsave('D:\\buffers\\%s-light_intensity.png'%(s), dataset.light_intensity, vmin=0.0, vmax=1.0)
            for i in range(5):
                plt.subplot(2, 3, i+1)
                plt.title(str(i+1))
                plt.imshow(LinearToSrgb(ToneMap(dataset.throughput[...,3*i:3*i+3])))
            plt.savefig('D:\\buffers\\%s-throughputs.png'%(s))
            # plt.imsave('D:\\buffers\\%s-throughput.png'%(s), dataset.throughput, vmin=0.0, vmax=1.0)

            for i in range(5):
                plt.subplot(2, 3, i+1)
                plt.title(str(i+1))
                plt.imshow(dataset.bounce_type[...,i])
            plt.savefig('D:\\buffers\\%s-bounce_types.png'%(s))
            # plt.imsave('D:\\buffers\\%s-bounce_type.png'%(s), dataset.bounce_type)

            for i in range(5):
                plt.subplot(2, 3, i+1)
                plt.title(str(i+1))
                plt.imshow((dataset.roughness[:,:,i] / (1 + dataset.roughness[:,:,i] / 1.5)) ** 0.45, cmap='binary')
            plt.savefig('D:\\buffers\\%s-roughnesses.png'%(s))
            # plt.imsave('D:\\buffers\\%s-roughness.png'%(s), dataset.roughness)

            for i in range(5):
                plt.subplot(2, 3, i+1)
                plt.title(str(i+1))
                plt.imshow(dataset.normals[...,3*i:3*i+3])
            plt.savefig('D:\\buffers\\%s-normals.png'%(s))

            for i in range(5):
                plt.subplot(2, 3, i+1)
                plt.title(str(i+1))
                plt.imshow(dataset.positions[...,3*i:3*i+3])
            plt.savefig('D:\\buffers\\%s-positions.png'%(s))
            break
    
    def test_preprocess():
        dataset = DenoiseDataset('D:\\SBMC', 8, mode='test', base_model='sbmc', use_g_buf=True, use_sbmc_buf=True, use_llpm_buf=True, sampling='grid')
        start = time.time()
        dataset._offline_preprocess(sbmc=True, llpm=True, kpcn=False, overwrite=True, remove_original=False)
        print(time.time() - start)
